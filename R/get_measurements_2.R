...
#'
#' If \code{parse} is FALSE, the JSON object is returned raw. For queries with multiple pages, a
#' list of such objects is returned. All pages are added to the list, even if there are no measurements.
#' The params \code{abridged} and \code{parse_datetime} have no effect.
#'
#'
#'
#' @details
...
#' }
#' @export
get_measurements_2 <- function(device_id,
                     date_from = NULL,
                     date_to = NULL,
                     page_size = 2000,
                     pages_per_query = 1,
  ...
  url <- paste0(.get_cumulocity_base_url(),
    "/measurement/measurements", collapse = "")
